...
const getListVideoByUsername = async (username) => {
    ...
    var loop = true
    while (loop) {
        listVideo = await page.evaluate((username) => {
            const listVideo = Array.from(document.querySelectorAll("a"));
            console.log(listVideo);
            const regex = new RegExp(`^https://www.tiktok.com/${username}/video/`);
            return listVideo
                .filter(item => regex.test(item.getAttribute('href')))
